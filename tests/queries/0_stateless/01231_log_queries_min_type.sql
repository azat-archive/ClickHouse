...
system flush logs settings log_queries=0;
select count() from system.query_log where query_id = previousQueryId();
...
system flush logs settings log_queries=0;
select count() from system.query_log where query_id = previousQueryId() and type = 'ExceptionWhileProcessing';
...
system flush logs settings log_queries=0;
select count() from system.query_log where query_id = previousQueryId() type = 'ExceptionWhileProcessing' and has(Settings.Names, 'max_rows_to_read');
